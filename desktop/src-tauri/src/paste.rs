...
pub async fn copy_item_native(
    ...
    crate::clipboard::pause_clipboard_watcher(600);
    copy_native(path.as_deref(), content.as_deref(), is_image)
}

#[tauri::command]
pub async fn copy_files_native(paths: Vec<String>) -> Result<(), String> {
    crate::clipboard::pause_clipboard_watcher(600);
    #[cfg(target_os = "windows")]
    unsafe {
        use std::os::windows::ffi::OsStrExt;
        ...
        for p in &paths {
            if std::path::Path::new(p).exists() {
                wide_paths.extend(std::ffi::OsStr::new(p).encode_wide());
                wide_paths.push(0);
            }
        }
        if !wide_paths.is_empty() {
            ...
            let df_size = std::mem::size_of::<DROPFILES>();
            let total_size = df_size + (wide_paths.len() * 2);
